    ...
    <div class="grid-version">
      ...
      <div class="version-list">
        ...
        <draggable :list="versionListArray" class="drag-area-version" :options="{group:{ name:'people',  pull:'clone', put:true }}">
          <div class="drag-item" v-for="(item, i) in versionListArray">
...
<script>
  ...
  export default {
    ...
    created: function created() {
      this.getPackageList();  // 获取应用的package列表
      this.getVersionList();  // 获取应用的补丁列表
    },
    methods: {
      getPackageList() {  // 获取package列表数据
        packageApi.list({ app_key: this.app_key, page: '' }).then((response) => {
      ...
      getVersionList() {  // 获取补丁列表数据
        versionApi.list({ app_key: this.app_key, page: '' }).then((response) => {
          if (response.errno === 0) { // 获取补丁列表成功
            this.versionListArray = response.data.data;  // 保存补丁列表数据
          } else {  // 获取补丁列表失败
            this.$message.error(response.errmsg);
          }
        })
